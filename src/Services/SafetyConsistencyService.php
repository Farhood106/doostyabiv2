...
class SafetyConsistencyService
{
    public function repairAndReport(): array
    {
        ...
        foreach ($pairs as $m) {
            $ok = $guard->canUsersSeeEachOther((int)$m['user_one_id'], (int)$m['user_two_id']);
            if ($ok) { continue; }
            ...
            $db->prepare("UPDATE match_cards SET hidden_until=DATE_ADD(NOW(), INTERVAL 10 YEAR), freshness_score=0 WHERE match_id=?")->execute([(int)$m['id']]);
            $hiddenCards += (int)$db->query('SELECT ROW_COUNT()')->fetchColumn();
        }

        return compact('hiddenMatches','hiddenChats','hiddenCards','mutualInvalidated');
    }
}
