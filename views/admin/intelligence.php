...
<section class="card table-wrap"><h2>معرفی‌های کم‌اطمینان</h2><p class="muted">این فهرست کمک می‌کند معرفی‌هایی را ببینید که داده کافی ندارند و ممکن است به شناخت‌نامه کامل‌تر یا توضیح ملایم‌تر نیاز داشته باشند.</p><table><thead><tr><th>شناسه</th><th>اعضا</th><th>سازگاری</th><th>اطمینان</th><th>وضعیت</th></tr></thead><tbody><?php foreach ($lowConfidenceMatches ?? [] as $match): ?><tr><td><?= (int)$match['id'] ?></td><td><?= e($match['user_one_name']) ?> ↔ <?= e($match['user_two_name']) ?></td><td><?= (int)round((float)$match['compatibility_score']) ?>٪</td><td><?= (int)round((float)$match['confidence_score']) ?>٪</td><td><?= e(fa_label($match['match_status'])) ?></td></tr><?php endforeach; ?></tbody></table><?php if (empty($lowConfidenceMatches)): ?><p class="muted">در حال حاضر معرفی کم‌اطمینان برجسته‌ای وجود ندارد.</p><?php endif; ?></section>
