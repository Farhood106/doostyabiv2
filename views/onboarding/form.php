...
?>
<section class="section-title"><div><h1>Onboarding</h1><p class="muted">Your previous answers are preserved. Update any section and save to continue.</p></div><span class="pill"><?= $percent ?>% complete</span></section>
<section class="card"><div class="progress"><span style="width: <?= $percent ?>%"></span></div><p class="help"><?= (int)$answeredQuestions ?> of <?= (int)$totalQuestions ?> questions answered.</p></section>
...
<?php foreach ($steps as $stepIndex => $step): ?><section class="card" id="step-<?= (int)$step['id'] ?>"><div class="section-title"><div><h2>Step <?= $stepIndex + 1 ?>: <?= e($step['title']) ?></h2><p><?= e($step['description']) ?></p></div><span class="pill"><?= count($step['groups']) ?> groups</span></div><?php foreach ($step['groups'] as $group): ?><fieldset><legend><?= e($group['title']) ?></legend><?php if ($group['description']): ?><p class="help"><?= e($group['description']) ?></p><?php endif; ?><?php foreach ($group['questions'] as $question): ?><div class="field"><label><?= e($question['title']) ?><?= $question['is_required'] ? ' *' : '' ?><?php if ($question['description']): ?><small><?= e($question['description']) ?></small><?php endif; ?><?= render_question_input($question, $cities, $existingAnswers[$question['id']] ?? null) ?></label><?php if (!empty($errors[$question['id']])): ?><span class="error-text"><?= e($errors[$question['id']]) ?></span><?php endif; ?><?php if ($question['help_text']): ?><small class="muted"><?= e($question['help_text']) ?></small><?php endif; ?></div><?php endforeach; ?></fieldset><?php endforeach; ?><button class="button secondary" type="submit" name="save_continue" value="<?= (int)$step['id'] ?>">Save and continue</button></section><?php endforeach; ?>
...
function choice_inputs(array $q, string $name, bool $multi, string $req, array $selected): string { $type = $multi ? 'checkbox' : 'radio'; $html = '<div class="checks">'; foreach ($q['options'] as $o) { $sel = in_array((int)$o['id'], $selected, true) ? ' checked' : ''; $desc = !empty($o['description']) ? '<small>'.e($o['description']).'</small>' : ''; $html .= '<label><input type="'.$type.'" name="'.$name.'" value="'.(int)$o['id'].'"'.$req.$sel.'> '.e($o['label']).$desc.'</label>'; } return $html.'</div>'; }
