...
## Phase 8.3 — Trust & Human Safety Layer

- Privacy Shield (`/safety/privacy-shield`): members can add protected identifiers (phone/email/full name/username). Values are normalized and hashed (SHA-256) for matching-time exclusion; raw values are not shown to admins.
- Bi-directional exclusion: shield checks are applied during candidate selection to prevent introductions when either side has a protected identifier match.
- Support/Advisor foundation: new support conversation architecture (`support_*` tables), member support inbox (`/support`), and admin support center (`/admin/support`) with assignment, closing, and quick replies.
